    ...
    for file in os.scandir("data"):
        try:
            if isinstance(parse(file.name), datetime.date):
                for country in countries_data["iso_codes"]:
                    try:
                        with open(f"data/{file.name}/{country}.json", "r") as current_country:
                            current_data = json.load(current_country)
                        data_cases_today[country].append(current_data["todayCases"])
                        data_cases[country].append(current_data["cases"])
                        data_deaths_today[country].append(current_data["todayDeaths"])
                        data_deaths[country].append(current_data["deaths"])
                        data_recovered_today[country].append(current_data["todayRecovered"])
                        data_recovered[country].append(current_data["recovered"])
